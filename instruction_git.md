...
## Вывод дерева коммитов

Для вывода дерева коммитов в уже знакомую команду git log необходимо добавить параметр --graph

    git log --graph

Также можно добавить и другие необходимые параметры для более нагрядной визуализации (например, --all и --oneline)

## Добавление картинки и .gitignore

Для добавления картинки в MarkDown используется формат

    ![picture](teftel.jpg)

В [] указывается сообщение, которое мы увидим, если картинка не прогрузится, а в () - путь и имя с расширением отображаемого файла. Если файл расположен в основной папке репозитория, можно написать только имя файла с расширением.

Обычно принято отслеживать только текстовые файлы, поэтому файлы с картинками и прочие ненужные файлы лучше убрать из отслеживаемых, то есть сделать исключение.

Для этого в репозитории создается файл
     
     .gitignore

в который вносятся названия тех файлов, которые мы не будем отслеживать. Также можно внести и маску файлов (добавить * перед точкой и расширение всех файлов, которые мы не хотим отслеживать). Например:

    *.jpg

Эта маска, прописанная в файле .gitignore в репозитории, позволит git не отслеживать все файлы с расширением .jpg.

_! Важно. Файл **.gitignore** также необходимо добавить к отслеживаемым. Только в этом случае исключения, прописанные в нем, будут учитываться._

# Третий семинар

## Удаленные репозитории

Строка с информацией (для ошибки)